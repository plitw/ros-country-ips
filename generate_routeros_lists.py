...
def generate_routeros_script(country_code, ip_list, is_ipv6=False, registry=""):
    ...
    with open(filename, "a") as file:
        ...
        file.write(f"{cmd_prefix} firewall address-list\n")

        if not exists:
            file.write(f"remove [find comment=\"{country_code}_{ip_version}\"]\n\n")
        
        for cidr in ip_list:
            file.write(f"add list=\"{country_code}_{ip_version}\" address={cidr} comment=\"{country_code}_{ip_version}\"\n")
