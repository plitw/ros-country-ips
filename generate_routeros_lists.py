...
def generate_routeros_script(country_code, ip_list, is_ipv6=False):
...
def main():
    country_data = process_apnic_data()
    
    for cc, data in country_data.items():
        if data["ipv4"]:
            generate_routeros_script(cc, data["ipv4"], is_ipv6=False)
            generate_plain_script(cc, data["ipv4"], is_ipv6=False)
        if data["ipv6"]:
            generate_routeros_script(cc, data["ipv6"], is_ipv6=True)
            generate_plain_script(cc, data["ipv6"], is_ipv6=True)
